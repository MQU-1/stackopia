# Migrating Expo UI to Stackopia Design System

Read `artifacts/stackopia-ds/docs/AGENTS.md` and
`artifacts/stackopia-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/stackopia-ds/lib/native-theme` and
  `@workspace/stackopia-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/stackopia-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/stackopia-ds` and retains only
